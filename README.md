...
- (optional and potentially dangerous) Run `docker system prune -a -f` remove all user files in docker to ensure that no previous attempts/containers are interfering with the install. **WARNING: THIS WILL ERASE ALL DOCKER CONTAINERS AND FILES YOU HAVE 
